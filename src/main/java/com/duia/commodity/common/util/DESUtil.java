...
import static com.duia.commodity.common.Constant.DES_KEY_ORDER;
...
public class DESUtil {
     ...
     * 解密并转换
     */
    public <E> E getDecryptParam(Class<E> clazz, String p) {
        String text;
        try {
            text = new DESUtil().strDec(p, DES_KEY_ORDER);

        } catch (Exception ex) {
            throw new IllegalArgumentException("加密参数解密异常>>>>>>>>>>>>>>>" + p);
        }

        try {
            return JSON.parseObject(text, clazz);
        } catch (Exception ex) {
            throw new IllegalArgumentException("解密参数JSON.parseObject转换异常>>>>>>>>>>>>>>>text:" + text + ">>>Class:" + clazz.getName());
        }
    }
